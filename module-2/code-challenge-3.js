/*
Scenario:
modify the ExtendedUser class (rewrite it) by adding a static match method to it. the method
should retrieve the teacher object, student object, and optionally a course name. its task is to
find the match between the student and the teacher.

in case the course name is not provided, the method should return:

    - an empty array if there is no match (the teacher does not teach courses the student is
      interested in, or teaches courses at a lower level)

    - an array with {course, level} objects, if the teacher teaches the courses the student is
      interested in.
...
test your solution using the follwing code:
